...
class Day20Runner: Runable
{
  class Node
  {
    ...
    func getNode(at indexFromSelf: Int, nrOfNodes: Int) -> Node
    {
      let absIndex: Int = abs(indexFromSelf)
      return stepGetNode(at: absIndex > (nrOfNodes/2) ? -indexFromSelf.signum() * (nrOfNodes - absIndex) : indexFromSelf)
    }
    func stepGetNode(at indexFromSelf: Int) -> Node
    {
      if indexFromSelf > 0 { return right.stepGetNode(at: indexFromSelf-1) }
      if indexFromSelf < 0 { return left.stepGetNode(at: indexFromSelf+1) }
      return self
    }
  }
  ...
  func mix(_ nodes: inout [Node])
  {
    for i in nodes.indices
    {
      ...
      { // Move right
        ...
        let target: Node = node.getNode(at: node.number%(nodes.count-1), nrOfNodes: nodes.count)
  ...
  func RunPart1(input: Input) -> String
  {
    ...
    mix(&nodes)
    return "\([Int](1...3).reduce(0, { $0 + nodes.first(where: { $0.number == 0 })!.getNode(at: ($1*1000) % nodes.count, nrOfNodes: nodes.count).number } ))"
  }
